...
import React, { useState, useTransition, useCallback, useEffect, createContext, useRef, useContext, useMemo } from 'react';
...
function Canvas(_a) {
    ...
    return (jsx("div", Object.assign({}, props, { ref: ref, children: jsx(CanvasContext.Provider, { value: { width, height }, children: jsx("svg", { width: width, height: height, children: children }) }) })));
}
const useCanvas = () => {
...
function ScrollingWindow(_a) {
    var { virtualWidth, virtualHeight } = _a, divProps = __rest(_a, ["virtualWidth", "virtualHeight"]);
    const styles = Object.assign({ overflow: 'scroll' }, divProps.style);
    return (jsx("div", Object.assign({}, divProps, { style: styles, children: jsx("div", { style: { width: virtualWidth, height: virtualHeight } }) })));
}
...
function ScrollableCanvas(_a) {
    ...
    return (jsxs("div", Object.assign({}, props, { children: [jsx(ScrollableCanvasContext.Provider, { value: { scrollLeft, scrollTop, setVirtualHeight, setVirtualWidth }, children: jsx(Canvas, { style: { height: '100%', width: '100%' }, children: children }) }), jsx(ScrollingWindow, { virtualWidth: virtualWidth, virtualHeight: virtualHeight, onScroll: onScroll, style: { height: '100%', width: '100%', position: 'absolute', top: '0', left: '0' } })] })));
}
...
const ScrollableCanvasContext = createContext(emptyScrollableCanvasContext);
...
const TimeScaleContext = createContext(undefined);
...
function CronoLayout({ children }) {
    ...
    return (jsx("g", { width: width, height: height, children: jsx(LayoutContext.Provider, { value: context, children: children }) }));
}
...
const TimeTicksContext = createContext(null);
...
function Tick({ tickHeight, text, margin = 3, textSize = 10, strokeWidth = 1, strokeColor = 'black', textColor = 'black', font = 'Arial', }) {
    ...
    return (jsxs("g", { children: [jsx("line", { y1: -tickHeight, y2: height - horizontalY, stroke: strokeColor, strokeWidth: strokeWidth }), jsx("text", { x: margin, y: -tickHeight, textAnchor: "start", alignmentBaseline: "hanging", fill: textColor, fontSize: textSize, fontFamily: font, children: text })] }));
}

function RegularTicks({ ticksX, ticksText, textSize, height, margin, strokeWidth }) {
    return (jsx("g", { children: ticksX.map((x, i) => (jsx("g", { transform: `translate(${x}, 0)`, children: jsx(Tick, { text: ticksText[i], textSize: textSize, tickHeight: height, margin: margin, strokeWidth: strokeWidth }, i) }, i))) }));
}
...
function makeStandardTicks({ type, timeScale }) {
    ...
    const StandardTicks = (props) => {
        return jsx(RegularTicks, Object.assign({ ticksX: ticksX, ticksText: ticksText }, props));
    };
...
function TimeTicks() {
    ...
    return (jsxs("g", { transform: `translate(${verticalX}, ${horizontalY})`, children: [showDays && jsx(DaysTicks, { textSize: fontSizeDays, height: daysPosition, margin: ticksTextMargin, strokeWidth: daysStrokeSize }), showWeeks && jsx(WeeksTicks, { textSize: fontSizeWeeks, height: weeksPosition, margin: ticksTextMargin, strokeWidth: weeksStrokeSize }), showMonths && jsx(MonthsTicks, { textSize: fontSizeMonths, height: monthsPosition, margin: ticksTextMargin, strokeWidth: monthsStrokeSize }), showYears && jsx(YearsTicks, { textSize: fontSizeYears, height: yearsPosition, margin: ticksTextMargin, strokeWidth: yearsStrokeSize })] }));
}

function CronodilePlanning() {
    return (jsx(ScrollableCanvas, { style: { height: '100%', width: '100%' }, children: jsx(CronoLayout, { children: jsx(TimeTicks, {}) }) }));
}

export { CronodilePlanning as default };
